...
Publicly reachable from the gateway are the callback `POST /api/v1/billing/webhooks/duitku` and JWT-protected transaction creation/list/get. An internal credential-protected transaction endpoint is intended for academic enrollment flow. Callback signature checks happen in the handler before the use case; the use case reconciles state/amount and activates an enrollment through academic after eligible payment (`internal/usecase/transaction_usecase.go:252-373`).
