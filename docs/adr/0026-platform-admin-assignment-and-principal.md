...
Apply migration 000002 before exposing platform routes. An operator can restore access after deactivation using the same command, but the deployment must control DB credentials and verify account ownership outside the API; see identity `PLATFORM_ADMIN.md`. Revocation removes access on the next `/platform/me` request even if the JWT remains cryptographically valid; no general JWT revocation is added.

> **Superseded in part by ADR 0032:** KEL-105 adds the platform-admin second factor this ADR deferred. Platform login now issues a five-minute pending JWT and requires TOTP verification before any privileged session; the assignment/principal and live-check design here is unchanged. The gateway alone cannot know whether an assignment was revoked: any future platform endpoint must repeat the live assignment check in identity or an equivalent trusted boundary. A DB outage on the live check fails closed with 503, not a platform grant. No production assignment or deployment was verified by this code review.
