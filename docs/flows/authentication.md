...
## Platform admin login and second factor (KEL-105)

**Implemented:** a platform admin signs in at `/platform/login`, which calls `POST /api/v1/platform/auth/login` through the gateway. A correct password on an active assignment returns a five-minute **pending** JWT (`platform_pending`, bound to the assignment's factor version), not a privileged session; the web action stores it only in a short-lived HttpOnly cookie scoped to `/platform`. `/platform/challenge` starts a challenge with purpose `verify` (or `enroll` after an operator ran `go run ./cmd/platform-admin -user-id <uuid>`, which re-enables exactly one enrollment and bumps `factor_version`, invalidating all older platform and pending JWTs). Identity's `POST /api/v1/platform/auth/challenge` creates a 256-bit challenge shown once to the browser and stored only as a SHA-256 hash, bound to user, purpose, and factor version, with a five-minute expiry and a five-attempt limit. `POST .../verify` consumes the challenge inside one transaction that row-locks the assignment and challenge rows, so two concurrent submissions cannot both succeed; only the stored AES-256-GCM-encrypted TOTP seed (keyed by `PLATFORM_FACTOR_KEY`) verifies the RFC 6238 code, and verifier or storage failure answers 503 with no password-only fallback. A successful verify issues a tenantless verified JWT with `platform_factor_version`; the web action then writes the `auth_token` session cookie, and `/platform` re-validates it through `GET /api/v1/platform/me` on every request. Identity rejects pending and legacy versionless platform tokens with 401 in its middleware and in the internal session checker, and the gateway applies the same rule, so no pre-verification or pre-rollout token reaches a platform route. Enrollment shows the setup key exactly once; logs, error responses, and browser persistence carry no seed, code, or challenge token. Evidence: identity PR #19 (squash `5ca2ddb1aee4946143e8ecacd5c18e4648bb9ed9`), gateway PR #19 (squash `bdbdee308c8c9dda722782d4c871b31876e4f9d7`), web PR #28 (squash `c259903f4e94deeb368b7e5bd3e75033eb019fcd`), `kelolakelas-identity-service/internal/{usecase/platform_factor.go,repository/platform_factor.go}`, [ADR 0032](../adr/0032-platform-admin-second-factor-totp.md).
