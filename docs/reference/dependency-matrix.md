...
| Academic | Identity | gRPC `:50051`, `tenant.PermissionService/CheckPermission` | current persisted role permission before catalog mutations | Implemented: `pkg/grpcclient/permission_client.go`; identity `internal/delivery/grpc/permission_service.go`; plaintext/auth hardening remains outstanding |
