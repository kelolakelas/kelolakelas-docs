...
|---|---|---|---|
| `kelolakelas-web` | `main` | `407563532c18f19a7cda16376db0dcfff846c2e5` | clean after KEL-31 |
...
**Implemented:** KelolaKelas is a Next.js App Router UI, a Gin reverse-proxy gateway, and separate identity, academic, and billing Go services. The code configures PostgreSQL per stateful service, Redis for gateway rate limiting and identity permission caching, identity gRPC on `:50051`, Duitku payment requests, Resend email, and optional Google Maps geocoding. Parent enrollment now uses the public catalog detail page to select an owned student and schedule before redirecting to the backend-provided checkout URL, and a parent can cancel their own pending enrollment, which withdraws the invoice and returns the seat. The gateway strips the client-supplied `X-Tenant-ID` and `X-Internal-Service-Credential` from every inbound request and republishes the tenant header from the verified JWT claim on protected routes. See [architecture](docs/02-architecture.md).
